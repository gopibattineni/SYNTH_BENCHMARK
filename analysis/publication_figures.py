...
def generate_publication_figures(
    ...
    if not scores.empty:
        ...
        ax.set_title("Figure 4: Bubble Chart (size = Fidelity Score)")
        ax.set_xlim(-0.05, 1.05)
        ax.set_ylim(-0.05, 1.05)
        ...
        if not ranking.empty:
            ...
            ax.set_xticks(angles[:-1])
            ax.set_xticklabels(labels)
            ax.set_title("Figure 5: All Generators")
        ...
        if not ranking.empty:
            ...
            ax.set_title("Figure 6: Overall Generator Ranking")
            ax.set_xlim(0, 1)
        ...
        ax.set_title("Figure 7: Generator Score Heatmap")
    ...
    if not scores.empty and scores["LeakageLevel"].nunique() > 1:
        for domain, col in [("Utility", "UtilityScore"), ("Privacy", "PrivacyScore"), ("Fidelity", "FidelityScore")]:
            ...
            ax.set_title(f"Figure 8: Leakage vs {domain}")
        ...
        for domain, col in [("Utility", "UtilityScore"), ("Privacy", "PrivacyScore"), ("Fidelity", "FidelityScore")]:
            ...
            ax.set_ylabel(f"{domain} Score")
            ax.set_title(f"Figure 8: {domain} by Generator (Leakage=0% baseline)")
    ...
    if not clf.empty:
        for metric in ["Accuracy", "F1"]:
            ...
            ax.set_title(f"Figure 9: TRTR vs TSTR ({metric})")
            ax.legend()
        ...
        ax.set_title("Figure 9: Utility Gap (TRTR − TSTR)")
        ax.tick_params(axis="x", rotation=45)
    ...
    if not clf.empty:
        for metric in ["Accuracy", "Precision", "Recall", "F1", "ROC-AUC"]:
            ...
            ax.set_title(f"Figure 10: {metric} by Classifier × Generator")
    ...
    if not scores.empty:
        for col, name, out_dir in [
            ...
            fig.suptitle(f"Figure 11: {name} Distribution Across Generators")
    ...
    if not scores.empty:
        ...
        if len(avail) >= 2:
            ...
            for method in ("pearson", "spearman"):
                ...
                ax.set_title(f"Figure 12: {method.capitalize()} Correlation")
    ...
    if not cd_ranks.empty:
        ...
        ax.set_title("Figure 13: Critical Difference Diagram (Friedman + Nemenyi)")
        ax.set_yticks([])
    ...
    if not ranking.empty:
        ...
        ax.set_xticklabels(labels)
        ax.set_ylim(0, 1)
        ax.set_title("Figure 14: Parallel Coordinates")
        ax.legend(bbox_to_anchor=(1.02, 1), fontsize=7)
        ax.grid(True, alpha=0.3)
