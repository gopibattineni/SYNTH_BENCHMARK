...
python run_experiment.py --dataset cancer --generator CTGAN --seed 68
...
def validate() -> dict:
    ...
    if not report["ok"]:
        raise SystemExit(
            "Validation failed: expected 15 datasets (9 clf + 6 reg), 8 generators, seeds [68,91,2025]"
        )
    return report
...
def main() -> None:
    ...
    if args.smoke:
        validate()
        status = run_one("cancer", "CTGAN", 68, force=args.force)
