# Leakage audit — Multi-seed Batch 2

**Original SYNTH pipeline was not modified.**  
Corrected Stage 1 lives only under `multi seed generators/second batch/`.
...
Split seed: **42** (fixed across generator seeds 68 / 91 / 2025).  
Test size: **0.20**.
...
* Batch 2: participant-level split **before** train-only imputation; overlap must be **0**

### 6. Repeated-entity datasets

| Dataset | Entity ID | Batch 2 split |
|---|---|---|
...
| Dataset | Rows (approx) | Unique entities | Existing split | Existing imputation | Potential leakage? | Final Batch 2 split | Imputer fitted on | Train / Test | Entity overlap | Status |
|---|---:|---:|---|---|---|---|---|---|---:|---|
...
Batch 2 reports fidelity, utility, and privacy using:
