# Leakage audit — Multi-seed Batch 1

**Original SYNTH pipeline was not modified.**  
Corrected Stage 1 lives only under `multi seed generators/1st batch/`.
...
Split seed: **42** (fixed across generator seeds 42 / 123 / 2024).  
Test size: **0.20**.
...
* Batch 1: participant-level split **before** train-only imputation; overlap must be **0**

### 6. Repeated-entity datasets

| Dataset | Entity ID | Batch 1 split |
|---|---|---|
...
| Dataset | Rows (approx) | Unique entities | Existing split | Existing imputation | Potential leakage? | Final Batch 1 split | Imputer fitted on | Train / Test | Entity overlap | Status |
|---|---:|---:|---|---|---|---|---|---|---:|---|
...
Batch 1 reports fidelity, utility, and privacy using:
