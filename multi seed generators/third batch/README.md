...
2. **4 generator-training seeds** — `[55, 155, 255, 355]` → Mean ± SD
...
> The same fixed split is reused for generator seeds 55, 155, 255, and 355 so that reported Mean ± SD reflects generator stochasticity, not split variability.
...
```bash
cd "SYNTH/multi seed generators/third batch"

python run_experiment.py --validate-only
python run_experiment.py --smoke      # Cancer × CTGAN × 4 seeds
python run_experiment.py --all        # resumable 480
...
```bash
cd "multi seed generators/third batch"
...
```text
multi seed generators/third batch/
├── classification/          ← 9 classification datasets (288 runs)
│   ├── results/raw|aggregated|logs
│   ├── figures/
│   ├── reports/
│   └── cache/splits/
├── regression/              ← 6 regression datasets (192 runs)
│   ├── results/raw|aggregated|logs
│   ├── figures/
│   ├── reports/
│   └── cache/splits/
