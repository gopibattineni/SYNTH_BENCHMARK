...
# Cap CPU side to 2 threads per worker (inherited by run_cuda_worker).
...
GENS = [
    ...
    "TabDDPM",
    "ForestDiffusion",
    "GaussianCopula",  # CPU-only; never claims a GPU slot
]
CPU_ONLY = {"GaussianCopula"}  # never pin to a GPU while GPU generators remain
...
def pending_jobs(prefer_gpu: bool = True) -> list[tuple[str, str]]:
    """Return (dataset, generator) pairs still needing work.

    prefer_gpu=True skips CPU-only generators (GaussianCopula) so free GPUs
    stay on real CUDA work. Call again with prefer_gpu=False only when the
    GPU queue is empty.
    """
    ...
    for ds in QUEUE:
        if ds in PROTECTED:
            continue
        for gen in GENS:
            if prefer_gpu and gen in CPU_ONLY:
                continue
            leftover = [
                s
                for s in SEEDS
...
while True:
    pending = pending_jobs(prefer_gpu=True)
    # Only schedule GaussianCopula once every GPU generator is queued/done.
    if not pending:
        pending = pending_jobs(prefer_gpu=False)
    ...
    if not pending and not busy:
        ...
        log("all Batch-3 jobs done; aggregating")
