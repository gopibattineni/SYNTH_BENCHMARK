...
dispatcher_alive() {
  local pid
  [[ -f "$BATCH/keep_four_busy.pid" ]] || return 1
  pid="$(cat "$BATCH/keep_four_busy.pid" 2>/dev/null || true)"
  [[ -n "$pid" ]] && kill -0 "$pid" 2>/dev/null
}

aggregator_alive() {
  local pid
  [[ -f "$BATCH/auto_aggregate.pid" ]] || return 1
  pid="$(cat "$BATCH/auto_aggregate.pid" 2>/dev/null || true)"
  [[ -n "$pid" ]] && kill -0 "$pid" 2>/dev/null
}

start_dispatcher() {
  setsid nohup bash "$BATCH/keep_four_busy.sh" </dev/null >/dev/null 2>&1 &
  echo $! > "$BATCH/keep_four_busy.pid"
  echo "$(date -Is) RESTART keep_four_busy pid=$(cat "$BATCH/keep_four_busy.pid")" >>"$WLOG"
}

start_aggregator() {
  setsid nohup bash "$BATCH/auto_aggregate_watch.sh" </dev/null >/dev/null 2>&1 &
