#!/usr/bin/env python3
"""Dual-panel forest plot of per-dataset OLS slopes (regression).

Reads all_datasets_statistical_summary.csv and writes:
  forest_plot_regression.{png,pdf}
...
FID_ANALYSIS = "fidelity_vs_r2_gap"
PRIV_ANALYSIS = "mia_vs_r2_gap"
...
def main() -> None:
    ...
    fig, axes = plt.subplots(1, 2, figsize=(12.0, 5.6), sharey=False, facecolor=FACE)
    fig.suptitle(
        r"Forest Plot — Regression"
        "\n"
        r"Regression Slopes with 95% Confidence Intervals",
        fontsize=14,
        fontweight="bold",
        color=INK,
        y=0.98,
    ...
    _draw_panel(axes[0], fid, r"Fidelity $\rightarrow$ $R^{2}$ Gap")
    _draw_panel(axes[1], priv, r"Privacy $\rightarrow$ $R^{2}$ Gap")
    ...
    fig.subplots_adjust(left=0.18, right=0.985, top=0.84, bottom=0.14, wspace=0.14)

    for ext in ("png", "pdf"):
        out = SCRIPT_DIR / f"forest_plot_regression.{ext}"
